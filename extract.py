...
MIN_SIZE = 3  # Minimum width or height in pixels
...
def extract_hitbox_rects(img_path, tol=TOLERANCE):
    ...
    for sl in find_objects(labeled):
        ...
        if w < MIN_SIZE or h < MIN_SIZE:
            continue  # Skip tiny rectangles
        rects.append([int(x0), int(y0), int(x1), int(y1)])
    # Only keep the largest rectangle (by area), or empty list if none
    if rects:
        rects = [max(rects, key=lambda r: (r[2]-r[0]) * (r[3]-r[1]))]
    return rects
